...
<template>
  <section id="perguntas-frequentes" class="max-w-4xl mx-auto px-4 pt-[5rem] lg:pt-[9rem] mb-24 bg-white">
    ...
    <div class="w-full max-w-4xl mx-auto">
      <div v-for="faq in faqs" :key="faq.id" class="border-b border-gray-200">
        <button @click="toggle(faq.id)" :aria-expanded="openItem === faq.id" :aria-controls="`faq-content-${faq.id}`"
          class="cursor-pointer w-full py-6 text-left text-xl font-medium text-gray-800 hover:text-[var(--color-primary)] focus:outline-none transition-colors duration-200 flex justify-between items-center">
          <span>{{ faq.question }}</span>
          <svg class="w-6 h-6 flex-shrink-0 transform transition-transform duration-200 text-gray-500"
            :class="{ 'rotate-180': openItem === faq.id }" fill="none" stroke="currentColor" viewBox="0 0 24 24"
            aria-hidden="true">
        ...
        <Transition enter-active-class="transition-all duration-300 ease-out overflow-hidden"
          ...
          <div v-if="openItem === faq.id" :id="`faq-content-${faq.id}`" role="region" class="overflow-hidden">
            <div class="pb-6">
              <p class="text-lg text-gray-700 leading-relaxed">{{ faq.answer }}</p>
            </div>
          </div>
        </Transition>
      </div>
